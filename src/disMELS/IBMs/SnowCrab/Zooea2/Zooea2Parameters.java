...
/**
 * DisMELS class representing parameters for snow crab zooea stage 2.
 * 
...
public class Zooea2Parameters extends AbstractLHSParameters {
     ...
     * Creates a new instance of EggStageParameters
     */
    public Zooea2Parameters(String typeName) {
    ...
    protected final void createMapToValues() {
        ...
        key = PARAM_minStageDuration;     setOfParamKeys.add(key); mapParams.put(key,new IBMParameterDouble(key,key,0.0006));
        key = PARAM_maxStageDuration;     setOfParamKeys.add(key); mapParams.put(key,new IBMParameterDouble(key,key,365.0));
        key = PARAM_minWeight;            setOfParamKeys.add(key); mapParams.put(key,new IBMParameterDouble(key,key,0.006));
        key = PARAM_randomizeTransitions; setOfParamKeys.add(key); mapParams.put(key,new IBMParameterBoolean(key,key,false));
        key = PARAM_initialWeight;        setOfParamKeys.add(key); mapParams.put(key,new IBMParameterDouble(key,key,0.0002));
    }
   ...
   public void selectIBMFunctionForCategory(String cat, String key){
    ...
    @Override
    public Object clone() {
        Zooea2Parameters clone = null;
        try {
            clone = (Zooea2Parameters) super.clone();
     ...
     */
    @Override
    public Zooea2Parameters createInstance(final String[] strv) {
        int c = 0;
        Zooea2Parameters params = new Zooea2Parameters(strv[c++]);
