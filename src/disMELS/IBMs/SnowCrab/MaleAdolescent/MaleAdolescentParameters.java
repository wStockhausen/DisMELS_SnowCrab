...
public class MaleAdolescentParameters extends AbstractLHSParameters {
    ...
    protected final void createMapToValues() {
        ...
        key = PARAM_maxStarvTime;         setOfParamKeys.add(key); mapParams.put(key, new IBMParameterDouble(key, key, 518400));
        key = PARAM_percLostWeight;       setOfParamKeys.add(key); mapParams.put(key, new IBMParameterDouble(key, key, 0.9385));
        key = PARAM_aLengthWeight;        setOfParamKeys.add(key); mapParams.put(key, new IBMParameterDouble(key, key, 0.00005));
        key = PARAM_bLengthWeight;        setOfParamKeys.add(key); mapParams.put(key, new IBMParameterDouble(key, key, 2.903));
        key = PARAM_confInt;              setOfParamKeys.add(key); mapParams.put(key, new IBMParameterDouble(key, key, 0.127));
    }
